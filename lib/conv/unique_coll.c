/**
 * Unique (non-repeated) record collector
 *
...
static bool
aushape_conv_unique_coll_is_valid(const struct aushape_conv_coll *coll)
{
    struct aushape_conv_unique_coll *unique_coll =
                    (struct aushape_conv_unique_coll *)coll;
    return aushape_gbuf_is_valid(&unique_coll->seen);
}
...
static void
aushape_conv_unique_coll_cleanup(struct aushape_conv_coll *coll)
{
    struct aushape_conv_unique_coll *unique_coll =
                    (struct aushape_conv_unique_coll *)coll;
    aushape_gbuf_cleanup(&unique_coll->seen);
}

static bool
aushape_conv_unique_coll_is_empty(const struct aushape_conv_coll *coll)
{
    struct aushape_conv_unique_coll *unique_coll =
                    (struct aushape_conv_unique_coll *)coll;
    return aushape_gbuf_is_empty(&unique_coll->seen);
}

static void
aushape_conv_unique_coll_empty(struct aushape_conv_coll *coll)
{
    struct aushape_conv_unique_coll *unique_coll =
                    (struct aushape_conv_unique_coll *)coll;
    aushape_gbuf_empty(&unique_coll->seen);
}
 ...
 */
static bool
aushape_conv_unique_coll_seen_has(struct aushape_conv_coll *coll,
                                  const char *name)
{
    struct aushape_conv_unique_coll *unique_coll =
                    (struct aushape_conv_unique_coll *)coll;
    ...
    assert(coll->type == &aushape_conv_unique_coll_type);
    assert(name != NULL);

    gbuf = &unique_coll->seen;
    p = gbuf->ptr;
 ...
 */
static bool
aushape_conv_unique_coll_seen_add(struct aushape_conv_coll *coll,
                                  const char *name)
{
    struct aushape_conv_unique_coll *unique_coll =
                    (struct aushape_conv_unique_coll *)coll;
    ...
    assert(coll->type == &aushape_conv_unique_coll_type);
    assert(name != NULL);
    return aushape_gbuf_add_buf(&unique_coll->seen, name, strlen(name) + 1);
}
 ...
 */
static bool
aushape_conv_unique_coll_add_field(struct aushape_gbuf *gbuf,
 ...
 */
static bool
aushape_conv_unique_coll_add_record(struct aushape_gbuf *gbuf,
                                    const struct aushape_format *format,
                                    size_t level,
                                    bool first,
                                    const char *name,
                                    auparse_state_t *au)
{
    ...
    do {
        ...
        if (strcmp(field_name, "type") != 0 &&
            ...
            GUARD_BOOL(aushape_conv_unique_coll_add_field(gbuf, format, l,
                                                          first_field,
                                                          field_name, au));
            first_field = false;
        }
...
static enum aushape_rc
aushape_conv_unique_coll_add(struct aushape_conv_coll *coll,
                             size_t level,
                             bool *pfirst,
                             auparse_state_t *au)
{
...
const struct aushape_conv_coll_type aushape_conv_unique_coll_type = {
    .size       = sizeof(struct aushape_conv_unique_coll),
    .init       = aushape_conv_unique_coll_init,
    .is_valid   = aushape_conv_unique_coll_is_valid,
    .cleanup    = aushape_conv_unique_coll_cleanup,
    .is_empty   = aushape_conv_unique_coll_is_empty,
    .empty      = aushape_conv_unique_coll_empty,
    .add        = aushape_conv_unique_coll_add,
};
