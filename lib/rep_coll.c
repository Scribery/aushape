                     ...
                     auparse_state_t *au)
{
    ...
    if (!aushape_gbuf_is_empty(&rep_coll->lines)) {
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_add_char(&rep_coll->lines, '\n'));
    }
    AUSHAPE_GUARD_BOOL(NOMEM, aushape_gbuf_add_str(&rep_coll->lines, raw));
    ...
    if (coll->format.lang == AUSHAPE_LANG_XML) {
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_space_opening(items,
                                                      &coll->format, l));
        AUSHAPE_GUARD_BOOL(NOMEM, aushape_gbuf_add_str(items, "<item>"));
    } else if (coll->format.lang == AUSHAPE_LANG_JSON) {
        if (!aushape_gbuf_is_empty(items)) {
            AUSHAPE_GUARD_BOOL(NOMEM,
                               aushape_gbuf_add_char(items, ','));
        }
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_space_opening(items,
                                                      &coll->format, l));
    ...
    if (coll->format.lang == AUSHAPE_LANG_XML) {
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_space_closing(items,
                                                      &coll->format, l));
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_add_str(items, "</item>"));
    } else if (coll->format.lang == AUSHAPE_LANG_JSON) {
        if (items->len > len) {
            AUSHAPE_GUARD_BOOL(NOMEM,
                               aushape_gbuf_space_closing(items,
    ...
    if (coll->format.lang == AUSHAPE_LANG_XML) {
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_space_opening(gbuf,
                                                      &coll->format, l));
        AUSHAPE_GUARD_BOOL(NOMEM,
                           aushape_gbuf_add_fmt(gbuf, "<%s raw=\"",
                                                rep_coll->name));
        AUSHAPE_GUARD_BOOL(NOMEM,
